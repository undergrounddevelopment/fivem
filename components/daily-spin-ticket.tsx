...
export function DailySpinTicket({ 
  ...
  const updateCountdown = () => {
    ...
    if (diff <= 0) {
      setCountdown("Ready!")
      fetchStatus() // Refresh status when ready
    } else {
  ...
  const fetchStatus = async () => {
    try {
      const res = await fetch("/api/spin-wheel/daily-status")
      if (res.ok) {
  ...
  const handleClaim = async () => {
    if (!status?.canClaim || claiming || !user) return
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (onClaimSuccess) {
          onClaimSuccess(data.newTickets)
        }
      } else {
