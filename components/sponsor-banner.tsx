...
export function SponsorBanner() {
  ...
  useEffect(() => {
    ...
    if (supabase) {
      try {
        channel = supabase
          .channel("banners:sidebar")
          .on(
            "postgres_changes",
            { event: "*", schema: "public", table: "banners", filter: "position=eq.sidebar" },
            () => fetchBanners(),
          )
          .subscribe()
      } catch (error) {
  ...
  return (
    ...
    >
      ...
      <div className="block relative group w-full overflow-hidden">
        {/* Proxy image to maintain dynamic height based on content */}
        {sponsor && (
          <img
