...
export async function updateUserMembership(userId: string, membership: string) {
  ...
  await db.query(
    'UPDATE users SET membership = $1, updated_at = NOW() WHERE discord_id = $2',
    [membership, userId]
  )
...
export async function updateUserAdmin(userId: string, isAdmin: boolean) {
  ...
  await db.query(
    'UPDATE users SET is_admin = $1, updated_at = NOW() WHERE discord_id = $2',
    [isAdmin, userId]
  )
...
export async function updateAnnouncement(id: string, data: { title?: string; content?: string; type?: string; active?: boolean }) {
  ...
  values.push(id)
  await db.query(
    `UPDATE announcements SET ${updates.join(', ')}, updated_at = NOW() WHERE id = $${paramIndex}`,
    values
  )
...
export async function getPendingAssets() {
  ...
  const result = await db.query(
    'SELECT * FROM assets WHERE active = false ORDER BY created_at DESC'
  )
  return result.rows
}
...
export async function approveAsset(assetId: string) {
  ...
  await db.query(
    'UPDATE assets SET active = true, updated_at = NOW() WHERE id = $1',
    [assetId]
  )
...
export async function approveThread(threadId: string) {
  ...
  await db.query(
    "UPDATE forum_threads SET status = 'approved', updated_at = NOW() WHERE id = $1",
    [threadId]
  )
...
export async function rejectThread(threadId: string) {
  ...
  await db.query(
    "UPDATE forum_threads SET status = 'rejected', updated_at = NOW() WHERE id = $1",
    [threadId]
  )
