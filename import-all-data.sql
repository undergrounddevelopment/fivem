-- Complete data import from backup
-- Generated: 2026-01-01T10:07:50.351Z
-- Total tables: 4
...
-- Import users data (1 statements)
...
-- Import assets data (10 statements)
...
-- Import forum_threads data (1 statements)
...
###   
### Want to contribute? We encourage everyone to upload scripts or helpful assets. Let’s grow this FiveM community together! Please ensure all uploads are:
...
-- Import messages data (2 statements)
...
SELECT setval('public.assets_id_seq', (SELECT COALESCE(MAX(id), 1) FROM public.assets));
SELECT setval('public.forum_threads_id_seq', (SELECT COALESCE(MAX(id), 1) FROM public.forum_threads));
