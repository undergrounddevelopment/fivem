...
export default function AssetView({ assetData: asset }) {
  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNewRating(0);
        setNewComment('');
        fetch(`/api/assets/${asset.id}/reviews`).then(res => res.json()).then(data => setReviews(data.reviews || []));
      } else {
              ...
              <div className="flex items-center gap-6 pt-4 border-t border-border/50">
                ...
                <div className="flex items-center gap-2 text-sm">
                  <div className="p-1.5 rounded-lg bg-amber-500/10">
                    <StarRating rating={asset.average_rating || 0} readOnly />
                  </div>
                  <span className="font-semibold">{asset.average_rating?.toFixed(1) || "N/A"}</span>
                  <span className="text-muted-foreground">({asset.rating_count || 0} reviews)</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <CardContent>
              ...
              <form onSubmit={handleReviewSubmit} className="mt-6">
                ...
                <textarea
                  ...
                  className="w-full mt-2 p-2 bg-secondary rounded-md"
                  rows={4}
                />
                <Button type="submit" disabled={isSubmittingReview} className="mt-2">Submit Review</Button>
              </form>
            </CardContent>
          </Card>
        </div>
